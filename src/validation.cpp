...
{
    CAmount ret = blockValue * 45 / 100; // 45%

    int nMNPIBlock = Params().GetConsensus().nMasternodePaymentsIncreaseBlock;
    int nMNPIPeriod = Params().GetConsensus().nMasternodePaymentsIncreasePeriod;

                                                                        // mainnet:
    if(nHeight > nMNPIBlock)                    ret += blockValue / 20; // 50.0% should create 45/45/10 distribution (needs to be confirmed by testing)
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    if(sporkManager.IsSporkActive(SPORK_3_INSTANTSEND_BLOCK_FILTERING)) {
        ...
        LogPrintf("CheckBlock(CHAINCOIN): spork is off, skipping transaction locking checks\n");
    }
